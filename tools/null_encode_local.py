...
def main(argv=None):
    ...
    out = a.out or encoder_dir(null.NAME) / "fields"
    t = time.time()
    m, label_map = null.load_model(a.weights, device=a.device, dtype=a.dtype)
    print(f"{m.folder.parent.name}/{m.folder.name} on {m.device}, {a.dtype}: loaded in {time.time() - t:.1f} s")
    checks = {}
    for ct in a.ct:
        u = ct.name.split(".")[0]
        tokens, labels, meta = null.encode(ct, m)
        ...
        if a.check:
            c = checks[u] = null.check_against_haversack(ct, labels, meta["grid"], label_map, a.weights, device=a.device)
            ...
            for k, v in sorted(c.items()):
                print(f"    {k:24} {v['mm']:6.2f} mm  LPS {v['lps_mm']}  volume x{v['volume_ratio']}")
    if checks:
        p = encoder_dir(null.NAME) / "geometry_check_local.json"
